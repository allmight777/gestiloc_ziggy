...
return new class extends Migration
{
    public function up(): void
    {
        if (DB::connection()->getDriverName() === 'mysql') {
            DB::statement("ALTER TABLE property_modification_audits MODIFY COLUMN status ENUM('modified', 'pending_approval', 'approved', 'rejected') NOT NULL DEFAULT 'modified'");
        } else {
            Schema::table('property_modification_audits', function (Blueprint $table) {
                $table->string('status', 50)->default('modified')->change();
            });
        }
    ...
    public function down(): void
    {
        ...
        if (DB::connection()->getDriverName() === 'mysql') {
            DB::statement("ALTER TABLE property_modification_audits MODIFY COLUMN status ENUM('pending_approval', 'approved', 'rejected') NOT NULL DEFAULT 'pending_approval'");
        } else {
            Schema::table('property_modification_audits', function (Blueprint $table) {
