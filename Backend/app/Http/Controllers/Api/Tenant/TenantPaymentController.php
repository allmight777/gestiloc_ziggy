...
class TenantPaymentController extends Controller
{
    ...
    {
        Log::info('[TenantPaymentController@dashboard] start', [
        ...
        if (!$tenant) {
            Log::warning('[TenantPaymentController@dashboard] tenant missing');
        ...
        $invoices = Invoice::whereHas('lease', fn($q) => $q->where('tenant_id', $tenant->id))
            ->get();

        // Calculate statistics
        $stats = [
            'total_paid' => $invoices->where('status', 'paid')->sum('amount_total'),
            'total_pending' => $invoices->whereIn('status', ['pending', 'sent'])->sum('amount_total'),
            'total_overdue' => $invoices->where('status', 'overdue')->sum('amount_total'),
            'payments_count' => $invoices->where('status', 'paid')->count()
        ];
        ...
        for ($i = 11; $i >= 0; $i--) {
            ...
            $monthlyInvoices = $invoices->filter(function($invoice) use ($month) {
                ...
                return $paidAt->format('Y-m') === $month->format('Y-m');
            });

            $chartData[] = [
                'month' => $monthName,
                'amount' => $monthlyInvoices->sum('amount_total'),
                'count' => $monthlyInvoices->count()
            ];
        }

        Log::info('[TenantPaymentController@dashboard] success', [
            'user_id' => $user->id,
            'stats' => $stats,
