...
> = {
  plumbing: {
    label: "Plomberie",
    icon: Droplet,
    hint: "Fuite, évier, WC, robinet...",
  },
  electricity: {
    label: "Électricité",
    icon: Zap,
    hint: "Prise, disjoncteur, lumière...",
  },
  heating: {
    label: "Chauffage",
    icon: Thermometer,
    hint: "Radiateur, chaudière, eau chaude...",
  },
  other: {
    label: "Autre",
    icon: HelpCircle,
    hint: "Autre problème dans le logement",
  },
};
...
function normalizeApiError(e: any, fallback: string) {
  if (e?.request && !e?.response)
    return "Le serveur ne répond pas. Vérifie ta connexion et réessaie.";
  const status = e?.response?.status;
  if (status === 401) return "Session expirée. Reconnecte-toi.";
  if (status === 403) return "Accès refusé.";
  if (status === 413)
    return "Fichiers trop volumineux. Réduis la taille des photos.";
  if (status === 422)
...
export const Interventions: React.FC<InterventionsProps> = ({ notify }) => {
  ...
  const propertyOptions = useMemo(() => {
    ...
    incidents.forEach((incident) => {
      ...
      const propertyName =
        (incident.property as any)?.name || lease?.property?.name;
      if (propertyName) {
  ...
  const onPickPhotos = (files: FileList | null) => {
    ...
    if (tooMany > 0)
      notify(
        "Maximum 8 photos. Les photos en trop ne seront pas ajoutées.",
        "info",
      );
    const maxSize = 5 * 1024 * 1024;
    const filtered = arr.filter((f) => f.size <= maxSize);
    if (filtered.length !== arr.length)
      notify("Certaines photos dépassent 5MB et ont été ignorées.", "info");
  ...
  const validate = (): FormErrors => {
    ...
    if (description.trim() && description.trim().length < 10) {
      errs.description =
        "Ajoute un peu plus de détails (au moins 10 caractères).";
    }
    const partialSlots = slots.some(
      (s) => (s.date || s.from || s.to) && !(s.date && s.from && s.to),
    );
    if (partialSlots)
      errs.slots =
        "Chaque créneau doit avoir une date + une heure de début + une heure de fin.";
    return errs;
  };
    ...
    try {
      ...
      if (status === 422 && fieldErrors) {
        ...
        if (fieldErrors.property_id)
          mapped.propertyId = fieldErrors.property_id?.[0] || "Bien invalide.";
        if (fieldErrors.title)
          mapped.title = fieldErrors.title?.[0] || "Titre invalide.";
        if (fieldErrors.description)
          mapped.description =
            fieldErrors.description?.[0] || "Description invalide.";
        if (fieldErrors.preferred_slots)
          mapped.slots =
            fieldErrors.preferred_slots?.[0] || "Créneaux invalides.";
        if (fieldErrors.photos)
          mapped.photos = fieldErrors.photos?.[0] || "Photos invalides.";
    ...
    if (searchQuery.trim()) {
      ...
      filtered = filtered.filter(
        (incident) =>
          ...
          leases
            .find((l) => l.property?.id === incident.property_id)
    ...
    if (selectedProperty !== "all") {
      filtered = filtered.filter((incident) => {
        const lease = leases.find(
          (l) => l.property?.id === incident.property_id,
        );
        const propertyName =
          (incident.property as any)?.name || lease?.property?.name;
        ...
        >
          ...
          >
            ...
            <p className="text-gray-600 mb-6">
              Vous n'avez pas de location active. Pour créer une intervention,
              vous devez avoir une location en cours.
            </p>
            <button
              onClick={() => setShowNoLeaseModal(false)}
          ...
          <div className="flex items-center justify-between">
            <div className="text-left">
              ...
              <p className="text-sm text-gray-600">
                Déclare un problème lié à ton bien
              </p>
            </div>
            <button
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              <div className="lg:col-span-1 space-y-4">
                ...
                <select
                  ...
                  className={inputBase}
                  style={{
                    borderColor: formErrors.propertyId ? "#ef4444" : "#e5e7eb",
                  }}
                >
                  {leases
                    ...
                    .map((l) => (
                      <option key={l.id} value={l.property?.id}>
                        {l.property?.name || "Bien"} - {l.property?.address} —{" "}
                        {l.property?.city}
                      </option>
                    ))}
                </select>
                {formErrors.propertyId ? (
                  <p className={errorText}>{formErrors.propertyId}</p>
                ) : null}
              ...
              <div className="lg:col-span-2 space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    ...
                    <p className={helperBase}>
                      Court et précis (ex: “Fuite robinet cuisine”).
                    </p>
                    <input
                      ref={titleRef}
                      value={title}
                ...
                <div>
                  ...
                  <p className={helperBase}>
                    Localisation, depuis quand, impact...
                  </p>
                  <textarea
                    ref={descriptionRef}
                    value={description}
                  ...
                  {formErrors.description ? (
                    <p className={errorText}>{formErrors.description}</p>
                  ) : null}
                </div>
                ...
                <div>
                  ...
                  {formErrors.photos ? (
                    <p className={errorText}>{formErrors.photos}</p>
                  ) : null}
                  ...
                  {photoFiles.length > 0 ? (
                    ...
                    <div className="mt-3 rounded-2xl border border-dashed border-gray-200 bg-gray-50 p-4 text-sm text-gray-600">
                      Ajoute des photos si possible (ça accélère la prise en
                      charge).
                    </div>
                  )}
                </div>
                ...
                <div>
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <div className="text-xs text-gray-600">
                        Propose 1 à 3 créneaux, si tu peux.
                      </div>
                    </div>
                  ...
                  {formErrors.slots ? (
                    <p className={errorText}>{formErrors.slots}</p>
                  ) : null}
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-6">
            <div>
              ...
              <p className="text-sm text-gray-400 mt-1 font-medium">
                Signalez et suivez vos demandes d'intervention
              </p>
            </div>
          </div>
          ...
          <div className="space-y-4">
            ...
            <Card className="p-4">
                ...
                <div className="relative w-48">
                  ...
                  >
                    <div className="flex items-center gap-2 truncate">
                      <Home
                        size={14}
                        className="text-[#70AE48] flex-shrink-0"
                      />
              ...
              <div className="mt-3 text-xs text-gray-500">
                {filteredIncidents.length} intervention
                {filteredIncidents.length > 1 ? "s" : ""} trouvée
                ...
                <table className="w-full">
                  <thead>
                    <tr className="border-b border-gray-200 bg-gray-50">
                      ...
                      <th className="text-center px-4 py-3 text-xs font-medium text-gray-700">
                        Action
                      </th>
                    </tr>
                  </thead>
                        ...
                        return (
                          ...
                          >
                            ...
                            <td className="px-4 py-3 text-sm">
                              ...
                              >
                                {priorityMeta[incident.priority]?.label ||
                                  incident.priority}
                              </span>
                            </td>
                            <td className="px-4 py-3 text-sm">
