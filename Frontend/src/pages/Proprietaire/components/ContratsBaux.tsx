...
import {
    ...
    Eye, FileSignature, UserCheck, Users
} from 'lucide-react';
...
const ContratsBaux: React.FC<ContratsBauxProps> = ({ notify }) => {
        ...
        try {
            ...
            const mapped = (data || []).map((l: any) => {
                ...
                return {
                    ...
                    depotGarantie: l.deposit ? new Intl.NumberFormat('fr-FR').format(parseInt(l.deposit)) + ' FCFA' : new Intl.NumberFormat('fr-FR').format(parseInt(l.rent_amount * 2)) + ' FCFA',
    ...
    const filtered = contrats.filter(c => {
        const matchesProperty = propertyFilter === 'all' ? true : c.bien.includes(properties.find(p => p.id.toString() === propertyFilter)?.name || '');
        const matchesSearch = c.titre.toLowerCase().includes(searchTerm.toLowerCase()) || c.bien.toLowerCase().includes(searchTerm.toLowerCase());
    ...
    const handleSign = async (uuid: string) => {
        try {
            setSigningId(uuid);
            await leaseService.signContract(uuid);
    ...
    const handleViewSigned = (uuid: string) => {
        ...
        const apiUrl = import.meta.env.VITE_API_URL || import.meta.env.VITE_API_URL || 'https://gestiloc-back.onrender.com/api';
            ...
            <div className="leases-container">
                <div className="leases-header">
                    <div className="header-content">
                        ...
                        >
                            Générez automatiquement vos contrats de bail personnalisés en quelques clics.
                            <br />
                    ...
                    <a
                        href="/proprietaire/nouvelle-location"
                        className="btn-new-lease"
                            ...
                            <div className="contract-card" key={c.id}>
                                ...
                                <div className="contract-footer">
                                    ...
                                    <div className="contract-actions">
                                        <button onClick={() => handleDownload(c.uuid)} disabled={downloadingId === c.uuid} className="action-btn btn-download" title="Télécharger le contrat">
                                        ...
                                        {c.statut === 'pending_signature' && (
                                            <>
                                                {!c.landlord_signature && (
                                                    <button onClick={() => handleSign(c.uuid)} disabled={signingId === c.uuid} className="action-btn btn-sign" title="Signer électroniquement">
                                                        {signingId === c.uuid ? <Loader2 size={14} className="animate-spin" /> : <PenSquare size={14} />}
                                                        <span>Signer</span>
                                                    </button>
                                                )}
                                                <button onClick={() => handleUploadClick(c)} disabled={uploadingId === c.uuid} className="action-btn btn-upload" title="Uploader un contrat signé">
                                        ...
                                        {c.signed_document && (
                                            <button onClick={() => handleViewSigned(c.uuid)} className="action-btn btn-view" title="Voir le contrat signé">
                                                <Eye size={14} />
                                                <span>Voir</span>
                                            </button>
                                        )}
